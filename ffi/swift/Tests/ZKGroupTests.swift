...
class ZKGroupTests: XCTestCase {
  ...
  let TEST_ARRAY_16_1: [UInt8] = [0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e, 0x6f, 0x70, 0x71, 0x72, 0x73]
  ...
  func testPniIntegration() throws {
    let aci: ZKGUuid       = try ZKGUuid(contents: TEST_ARRAY_16)
    let pni: ZKGUuid       = try ZKGUuid(contents: TEST_ARRAY_16_1)
    ...
    // Gets stored profile credential
    let clientZkGroupCipher  = ClientZkGroupCipher(groupSecretParams: groupSecretParams)
    let pniCredential = try clientZkProfileCipher.receivePniCredential(pniCredentialRequestContext: context, pniCredentialResponse: response)

    let presentation = try clientZkProfileCipher.createPniCredentialPresentation(randomness: TEST_ARRAY_32_5, groupSecretParams: groupSecretParams, pniCredential: pniCredential)

    XCTAssertEqual(presentation.serialize(), pniPresentationResult)

    // Verify presentation
    try serverZkProfile.verifyPniCredentialPresentation(groupPublicParams: groupPublicParams, pniCredentialPresentation: presentation)

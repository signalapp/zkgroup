...
public class PniCredentialRequestContext : ByteArray {
  ...
  public static let SIZE: Int = 489

  public init(contents: [UInt8]) throws  {
    try super.init(newContents: contents, expectedLength: PniCredentialRequestContext.SIZE)

    
    let ffi_return = FFI_PniCredentialRequestContext_checkValidContents(self.contents, UInt32(self.contents.count))
  ...
  public func getRequest() throws  -> ProfileKeyCredentialRequest {
    ...
    let ffi_return = FFI_PniCredentialRequestContext_getRequest(self.contents, UInt32(self.contents.count), &newContents, UInt32(newContents.count))
